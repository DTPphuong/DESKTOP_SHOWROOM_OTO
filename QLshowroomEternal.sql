...
Create table NhanVien(
 ...
 alter table HoaDon  add constraint fk_MaSP foreign key(MaSP) references SanPham(masp)
...
Select * from SanPham;
  ...
  ------==========================================================
  create proc DoanhThuNam
  @nam int as 
  select year(NgayLap)  as 'năm' ,sum(TongTien)  as 'Tổng tiền' ,count(MaHD) as 'Tổng hóa đơn trong năm' from HoaDon 
    where  year(NgayLap) = @nam 
  group by month(NgayLap) , year(NgayLap)
  
  --- run 
  exec DoanhThuNam 2021;
  
  
   create proc DoanhThuThang
  @thang  int , @nam int as 
  select day(NgayLap) as 'Ngày', month(NgayLap)as 'tháng' , year(NgayLap)  as 'năm' ,sum(TongTien)  as 'Tổng tiền'  ,count(MaHD) as 'Tổng hóa đơn trong tháng' from HoaDon 
    where  year(NgayLap) = @nam and month(NgayLap) = @thang
  group by day(NgayLap), month(NgayLap) , year(NgayLap)

  --- run 
  exec DoanhThuThang 7,2021;


   create proc DoanhThuNgay
 @ngay int, @thang  int , @nam int as 
  select day(NgayLap) as 'Ngày' , month(NgayLap)as 'tháng' , year(NgayLap)  as 'năm' ,sum(TongTien)  as 'Tổng tiền' ,count(MaHD) as 'Tổng hóa đơn trong ngày'from HoaDon 
     where  year(NgayLap) = @nam and month(NgayLap) = @thang and day(NgayLap) = @ngay
  group by day(NgayLap), month(NgayLap) , year(NgayLap)
   
      
  --- run 
  exec DoanhThuNgay 14,4,2021;
    exec DoanhThuThang 12,2021;
	exec DoanhThuNam 2021;
